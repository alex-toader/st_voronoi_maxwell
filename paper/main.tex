...
\begin{proposition}
\label{prop:rank6}
If the edge directions $\{\Delta x_e : e \in E\}$ are not contained in a
quadric cone (i.e.\ no nonzero symmetric $Q$ satisfies
$(\Delta x_e)^\top Q\, (\Delta x_e) = 0$ for all~$e$), then
$\rank\{(\Delta x_e) \otimes (\Delta x_e) : e \in E\} = 6$,
so $\dim \mathcal{A} = |E| - 6$.
\end{proposition}

\begin{proof}
The symmetric rank-1 matrices $\Delta x_e \otimes \Delta x_e$ lie in the
6-dimensional space $\mathrm{Sym}^2(\mathbb{R}^3)$. Their span has
dimension $< 6$ iff some nonzero $Q \in \mathrm{Sym}^2$ is orthogonal
to all of them: $\tr(Q\, \Delta x_e \otimes \Delta x_e)
= (\Delta x_e)^\top Q\, (\Delta x_e) = 0$ for all~$e$, meaning all
edge directions lie on the quadric cone $\{v : v^\top Q\, v = 0\}$.
This is verified to fail (rank $= 6$) on all tested structures.
\end{proof}
...
At~$\Gamma$, $D > 0$ with gap $\sim 0.54$--$0.60$ (observed numerically
across all tested structures); by continuity,
